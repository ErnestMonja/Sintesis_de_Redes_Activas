...
\begin{equation}
    \label{eq_2}
    T_{PB}(s) = \frac{3,284x10^{7}}{s^2 + 3184s + 6,632x10^{7}}
\end{equation}

\begin{equation}
    \label{eq_3}
    T_{PA}(s) = \frac{0,5s^{2}}{s^2 + 1896s + 2,35x10^{7}}
\end{equation}
...
\begin{equation}
    \label{eq_49}
    \omega_{p} = \sqrt{\frac{1}{R_{1}R_{2}C_{1}C_{2}}}
\end{equation}
...
\begin{equation}
    \label{eq_50}
    S_{R_{1}}^{\omega_{p}} = \frac{R_{1}}{\omega_{p}}\frac{\partial \omega_{p}}{\partial R_{1}} 
    = \frac{R_{1}}{\sqrt{\frac{1}{R_{1}R_{2}C_{1}C_{2}}}} \frac{\partial}{\partial R_{1}}\left(\sqrt{\frac{1}{R_{1}R_{2}C_{1}C_{2}}}\right) 
\end{equation}
\begin{equation}
    \label{eq_51}
    S_{R_{1}}^{\omega_{p}} = R_{1} \sqrt{R_{1}}\sqrt{R_{2}C_{1}C_{2}} \frac{\partial}{\partial R_{1}} \left(\frac{1}{\sqrt{R_{1}}} \right) \left(\frac{1}{\sqrt{R_{2}C_{1}C_{2}}}\right)
\end{equation}
\begin{equation}
    \label{eq_52}
    S_{R_{1}}^{\omega_{p}} = R_{1} \sqrt{R_{1}} \left(-\frac{1}{2R_{1}\sqrt{R_{1}}}\right) = - \frac{1}{2}
\end{equation}

Se tiene que este resultado se comprueba para todas las sensibilidades del polo $\omega_{p}$ con respecto a cada constante del circuito, tal que:

\begin{equation}
    \label{eq_53}
    S_{R_{1}}^{\omega_{p}} = S_{R_{2}}^{\omega_{p}} = S_{C_{1}}^{\omega_{p}} = S_{C_{2}}^{\omega_{p}} = - \frac{1}{2}
\end{equation}
...
Para realizar las sensibilidades con respecto al ancho de banda, primero debemos definir al factor de calidad de los polos $Q_{p}$ y para ello igualaremos el valor encontrado en la ecuación (\ref{eq_16}) con el valor genérico que tiene el termino relacionado a $s$ en un filtro genérico, es decir que:

\begin{equation}
    \label{eq_56}
    \frac{\omega_{p}}{Q_{p}} = \frac{1}{R_{1}C_{1}} + \frac{1}{R_{2}C_{1}} + \frac{1-k}{R_{2}C_{2}}
\end{equation}
...
\begin{equation}
    \label{eq_57}
    S_{R_{1}}^{\frac{\omega_{P}}{Q_{p}}} = -\frac{1}{2} + Q_{p}\sqrt{\frac{R_{2}C_{2}}{R_{1}C_{1}}}
\end{equation}

\begin{equation}
    \label{eq_58}
    S_{R_{2}}^{\frac{\omega_{P}}{Q_{p}}} = -\frac{1}{2} + Q_{p}\left( \sqrt{\frac{R_{1}C_{2}}{R_{2}C_{1}}} + (1-k) \sqrt{\frac{R_{1}C_{1}}{R_{2}C_{2}}}\right)
\end{equation}

\begin{equation}
    \label{eq_59}
    S_{C_{1}}^{\frac{\omega_{P}}{Q_{p}}} = -\frac{1}{2} + Q_{p}\left( \sqrt{\frac{R_{1}C_{2}}{R_{2}C_{1}}} + \sqrt{\frac{R_{2}C_{2}}{R_{1}C_{1}}}\right)
\end{equation}

\begin{equation}
    \label{eq_60}
    S_{C_{2}}^{\frac{\omega_{P}}{Q_{p}}} = -\frac{1}{2} + (1-k)Q_{p} \sqrt{\frac{R_{1}C_{1}}{R_{2}C_{2}}}
\end{equation}

Se tiene que para definir la sensibilidad de $k$ con respecto al $\frac{\omega_{p}}{Q_{p}}$, se cumple ahora que según (\ref{eq_56}), existe una relación entre $k$ y el ancho de banda. Para calcularla, se tiene que podemos partir de la parte a derivar de la sensibilidad, es decir:

\begin{equation}
    \label{eq_61}
    \frac{\partial}{\partial k} \left(\frac{1}{R_{1}C_{1}} + \frac{1}{R_{2}C_{1}} + \frac{1-k}{R_{2}C_{2}}\right) = \frac{\partial}{\partial k} \left(\frac{1-k}{R_{2}C_{2}}\right) = -\frac{1}{R_{2}C_{2}}
\end{equation}

Resulta entonces que la sensibilidad es igual a:

\begin{equation}
    \label{eq_62}
    S_{k}^{\frac{\omega_{p}}{Q_{p}}} = \frac{k}{\frac{\omega_{p}}{Q_{p}}} \frac{\partial}{\partial k} \left(\frac{\omega_{p}}{Q_{p}}\right) = \frac{-\frac{k}{R_{2}C_{2}}}{\frac{\omega_{p}}{Q_{p}} = \frac{1}{R_{1}C_{1}} + \frac{1}{R_{2}C_{1}} + \frac{1-k}{R_{2}C_{2}}} 
\end{equation}


\subsection{Análisis de la Peor Desviación}
Para este análisis, podemos analizar al conjunto de ambos filtros como uno solo, donde si definimos a la frecuencia del polo como la media geométrica entre la frecuencia del polo del Filtro Pasa Bajo como la del Filtro Pasa Alto, es decir
...
Se tiene entonces que el desplazamiento relativo total de la frecuencia central y de las etapas, considerando variaciones pequeñas y simultáneas en los componentes, se aproxima por:
...
close all; clear all; clc;        % Trabajo Practico 4 - Filtros Activos
