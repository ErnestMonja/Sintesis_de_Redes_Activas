import numpy as np
import control as ctl
import matplotlib.pyplot as plt

# VFA (LM324)
Ad0 = 1e5
fp3 = 10
fp4 = 5.06e6

# CFA (LM6181)
fp1 = 14e3
fp2 = 82.3e6
RT = 2.37e6
CT = 4.8e-12

# Resistencias del circuito
R2 = 850
R1 = 20.238   
Rf = 90e3
Ri = 10e3

s = ctl.TransferFunction.s

wp1 = 2*np.pi*fp1
wp2 = 2*np.pi*fp2
wp3 = 2*np.pi*fp3
wp4 = 2*np.pi*fp4

# Parámetros de la red
Cx = 10e-12        
Rx = 3145.355          
Ry = 3145.355          

s = ctl.TransferFunction.s

# Función de transferencia dela red
Ac = (Ry/(Ry + Rx)) * ((1 + s*Cx*Rx)/(1 + s*Cx*(Rx*Ry/(Rx + Ry))))
Ac = ctl.minreal(Ac, verbose=False)

# CFA
Avfi2 = 1 + R2/R1
ZT = RT/((1 + s/(wp1))*(1 + s/wp2))
T2 = -ZT/R2
Avf2 = Avfi2/(1 - 1/T2)

# Ganancia a lazo abierto total
Av = Ad0 / ((1 + s/wp3)*(1 + s/wp4)) * Avf2 * Ac

# Ganancia de lazo
T = - Av / (1 + Rf/Ri)
T = ctl.minreal(T, verbose=False)

# Ganancia a lazo cerrado
Avf = Av / (1 - T)
Avf = ctl.minreal(Avf, verbose=False)

# Polos y ceros
ceros_Ac = ctl.zeros(Avf)
polos_Ac = ctl.poles(Avf)

print("\nCeros:")
for c in ceros_Ac:
    freq = abs(c)/(2*np.pi)/1e6
    print(f"{freq} MHz")


print("Polos:")
for p in polos_Ac:
    freq = abs(p)/(2*np.pi)/1e6
    print(f"{freq} MHz")

# Ganancia Estática
K_Avf = ctl.dcgain(Avf)
print("K =", K_Avf, "veces")

# Bode Avf
w = np.logspace(4, 11, 3000)
plt.figure()
ctl.bode_plot(Avf, w, dB=True)
plt.suptitle("Avf compensado")
plt.show()

# Bode T
w = np.logspace(4, 11, 3000)
plt.figure()
ctl.bode_plot(T, w, dB=True)
plt.suptitle("Ganancia de Lazo T")
plt.show()

# Margen de fase
gm, pm, wg, wp = ctl.margin(T)  
print(f"Margen de fase = {180 + pm:.2f}°")
